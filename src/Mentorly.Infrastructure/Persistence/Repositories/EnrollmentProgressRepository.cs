...
public sealed class EnrollmentProgressRepository(MentorlyDbContext dbContext) : IEnrollmentProgressRepository
{
    ...
    public async Task<IReadOnlyList<Guid>> GetMandatoryActivityIdsAsync(Guid courseId, CancellationToken cancellationToken = default) => await dbContext.Activities.Where(x => x.Theme.Unit.CourseId == courseId && x.IsMandatory && x.Type == ActivityType.Exercise).Select(x => x.Id).ToListAsync(cancellationToken);
}
