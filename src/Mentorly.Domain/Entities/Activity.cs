...
{
    private Activity() { }
    public Activity(Guid id, Guid themeId, string title, string description, ActivityType type, bool isMandatory, ApprovalStrategy approvalStrategy, int orderIndex)
    {
        if (id == Guid.Empty || themeId == Guid.Empty) throw new ArgumentException("Activity and theme ids are required.");
        Id = id; ThemeId = themeId; Update(title, description, type, isMandatory, approvalStrategy, orderIndex);
    }
    ...
    public void Update(string title, string description, ActivityType type, bool isMandatory, ApprovalStrategy approvalStrategy, int orderIndex)
    {
        ...
        Title = title.Trim(); Description = (description ?? string.Empty).Trim(); Type = type; IsMandatory = isMandatory; ApprovalStrategy = approvalStrategy; OrderIndex = orderIndex;
    }
